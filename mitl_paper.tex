...
C1 (saturation bounds) accounts for 18\,074 of 18\,291 total violations and is
the primary driver of perfect recall.  C2 (rate limiter) contributes 216
violations at eTaPR-F1 = 0.844.  C3 (SP/PV tracking) fires zero times on the
HAI 22.04 test split: the four test files do not contain AP27-type attacks
(frozen SIT01 while AutoSD ramps), confirming that C3 is selective — it fires
only when the physical invariant is violated, not on other attack classes.
This selective behaviour is the desired property of a specification-derived
constraint: it does not over-generalise.
